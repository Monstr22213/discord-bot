...
RUN echo "cache bust $RAILWAY_DEPLOYMENT_ID $RAILWAY_GIT_COMMIT_SHA $(date +%s)" > /tmp/bust && pip install --no-cache-dir --upgrade pip && pip uninstall -y discord.py discord 2>/dev/null; pip install --no-cache-dir --force-reinstall -r requirements.txt && python -c "import discord; print(f'discord {discord.__version__} sinks={__import__(\"importlib\").util.find_spec(\"discord.sinks\") is not None}')"
COPY . .
CMD ["python", "bot.py"]
